        ...
        <div className="space-y-4">
          {layers.map((layer, idx) => (
            <motion.div key={layer.id} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: idx * 0.1 }}
              className={`p-8 border rounded-sm transition-all group ${idx === 2 ? 'border-cyan-500/30 bg-cyan-500/[0.02]' : 'border-zinc-900 bg-zinc-950/30 hover:border-zinc-700'}`}>
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="flex items-start gap-6">
                  <span className="font-mono text-[10px] text-zinc-700 mt-1">[{layer.id}]</span>
                  <div>
                    <h3 className={`text-sm font-bold tracking-widest uppercase mb-2 ${idx === 2 ? 'text-cyan-400' : 'text-zinc-200'}`}>{layer.title}</h3>
