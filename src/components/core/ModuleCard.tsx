...
const ModuleCard = ({ id, title, description, status: initialStatus, type }: any) => {
  ...
  useEffect(() => {
    const logs: any = {
      auditor: ["SCANNING_PULSE...", "ANALYZING_SEMANTICS...", "COLLUSION_CHECK: PASS", "SIGNING_HASH: 0x82f..."],
      nist: ["CONTROL_AC-3: OK", "CONTROL_IA-2: OK", "ENFORCING_LEAST_PRIVILEGE...", "AUDIT_LOG_VERIFIED"],
      crypto: ["BLOCK_HEIGHT: 18.2M", "VERIFYING_MERKLE_ROOT...", "INTEGRITY: 100%", "TX_TRACE_COMPLETE"]
    };

    const interval = setInterval(() => {
      if (logs[type]) {
        const nextLog = logs[type][Math.floor(Math.random() * logs[type].length)];
  ...
  return (
    <div className="bg-zinc-950 border border-zinc-900 p-5 rounded-2xl group hover:border-emerald-500/30 transition-all">
      ...
      <div className="bg-black/50 rounded-lg p-3 border border-zinc-900/50 min-h-[40px] flex items-center">
