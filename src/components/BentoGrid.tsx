...
import { Shield, Brain, Lock, Activity, Cpu } from 'lucide-react';

const tiles = [
  { id: "01", title: "Sentient Sync Core", desc: "Autonomous AGI architecture for deterministic orchestration.", icon: <Brain size={20} className="text-cyan-500" />, size: "md:col-span-2 md:row-span-2", status: "ACTIVE" },
  ...
  { id: "03", title: "CryptoGuard AI", desc: "Forensic monitoring for smart contract integrity.", icon: <Lock size={20} className="text-purple-500" />, size: "col-span-1 row-span-1" },
  { id: "04", title: "NIST Compliance", desc: "Zero-trust logic gates and incident response alignment.", icon: <Cpu size={20} className="text-blue-500" />, size: "col-span-1 row-span-1" },
  { id: "05", title: "Audit Persistence", desc: "SQLite-backed storage for AGI transparency.", icon: <Activity size={20} className="text-zinc-500" />, size: "col-span-1 row-span-1" }
];
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 auto-rows-[200px]">
          {tiles.map((tile, index) => (
            <motion.div key={index} whileHover={{ y: -2 }} className={`${tile.size} group relative p-6 rounded-sm border border-zinc-900 bg-zinc-950/50 flex flex-col justify-between hover:border-zinc-700 transition-all overflow-hidden`}>
